...
DISCLAIMER = (
    "ℹ️ <b>Важно</b>\n"
    "Бот показывает данные из открытого источника <b>gdebenz.ru</b> "
    "(наличие топлива вносят сами пользователи этого сайта). "
    "Точность и актуальность информации не гарантируется — "
    "уточняйте наличие топлива на месте.\n"
    "Для работы команд <code>/history</code> и статистики бот хранит "
    "минимум технических данных: ваш Telegram ID, имя, username и "
    "список запрошенных городов. Эти данные не передаются третьим лицам."
)
...
async def cmd_start(message: Message):
    ...
    text = (
        ...
        "Напишите название города — например, <b>Александров</b> или <b>Владимир</b> — "
        "и я предложу выбрать режим показа.\n\n"
        "/help — справка\n\n"
        f"{DISCLAIMER}"
    )
    if last_city:
...
async def cmd_help(message: Message):
    ...
    await message.answer(
        ...
        "Просто напишите город — например, <b>Александров</b>. "
        "Бот спросит, что показать:\n"
...
async def cmd_privacy(message: Message):
    _track(message)
    await message.answer(
        "🔒 <b>Данные и ответственность</b>\n\n"
        "<b>Источник информации.</b> Бот только отображает данные из открытого "
        "стороннего сервиса <b>gdebenz.ru</b>, где статус АЗС отмечают сами "
        "пользователи этого сайта. Бот не является владельцем и не проверяет "
        "эти данные.\n\n"
        "<b>Ответственность.</b> Точность, актуальность и полнота информации "
        "о наличии топлива не гарантируется. Перед поездкой уточняйте наличие "
        ...
        "<b>Что хранит бот.</b> Для работы команд <code>/history</code> и "
        "статистики сохраняются: Telegram ID, имя и username, а также список "
        "запрошенных вами городов и время запроса. Пароли, платёжные и "
        "иные персональные данные бот не запрашивает и не хранит.\n\n"
        "<b>Передача третьим лицам.</b> Собранные данные не продаются и не "
        "передаются третьим лицам, используются только для работы бота.",
    )
