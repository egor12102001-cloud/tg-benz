...
from aiogram.types import (
    ...
    BotCommandScopeChat, CallbackQuery, InlineKeyboardButton, InlineKeyboardMarkup,
...
def _mode_kb(city: str) -> InlineKeyboardMarkup:
    """Выбор режима — показывается после ввода города."""
...
def _refresh_kb(city: str, mode: str) -> InlineKeyboardMarkup:
    """Кнопка обновления под результатами."""
    return InlineKeyboardMarkup(inline_keyboard=[[
...
async def _fetch_and_render(
    ...
    if result.error:
        log_query(uid, cid, ctype, city, success=False, error=result.error)
    else:
        log_query(uid, cid, ctype, city, success=True, stations=len(result.stations))
...
async def cmd_fuel(message: Message):
    ...
    try:
        chunks, kb = await _fetch_and_render(city, "all", uid, message.chat.id, message.chat.type)
...
async def cmd_fuelnow(message: Message):
    ...
    msg = await message.answer(f"⏳ Загружаю АЗС для <b>{city}</b>...")
    try:
        chunks, kb = await _fetch_and_render(city, "now", uid, message.chat.id, message.chat.type)
    except Exception as e:
        await msg.edit_text(f"❌ Ошибка: {e}")
        return
    no_preview = LinkPreviewOptions(is_disabled=True)
...
async def msg_text_private(message: Message):
    ...
    if not city:
        return
    _track(message)
    # Callback data limit = 64 bytes; обрезаем город если нужно
    city_safe = city[:40]
    await message.answer(
